...
export function MarketInsights() {
  return (
    <section
      id="insights"
      className="relative border-t border-white/10 bg-[#070707] py-16 sm:py-28"
    >
      <div className="mx-auto block max-w-7xl px-5 sm:hidden">
        ...
        <div className="mt-5 grid gap-4">
          {marketInsights.map((item) => (
            <article key={item.label} className="border border-white/10 bg-black p-5">
              <p className="text-xs uppercase tracking-[0.22em] text-amber-200/80">
                {item.label}
              </p>
      ...
      <div className="relative z-10 mx-auto hidden max-w-7xl px-5 sm:block sm:px-8 lg:px-10">
        <div className="grid gap-6 sm:gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:gap-12">
          <div className="safari-paint-layer relative transform-gpu rounded-[2.5rem] border border-white/12 bg-[#151515] p-10 shadow-[0_40px_120px_-80px_rgba(0,0,0,0.65)]">
          ...
          <div className="grid gap-5">
            {marketInsights.map((item) => (
              <article
                key={item.label}
                className="safari-paint-layer relative transform-gpu rounded-[2rem] border border-white/10 bg-zinc-950 p-7 shadow-2xl shadow-black/30"
              >
